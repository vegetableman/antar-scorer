 ...
 */
function isProbablyReaderable(doc: Document, isVisible?: Function): boolean {
  if (!isVisible) {
    isVisible = isNodeVisible;
  }

  let nodes = Array.from(doc.querySelectorAll("p, pre"));
  ...
  if (brNodes.length) {
    ...
    [].forEach.call(brNodes, function(node: HTMLElement) {
      set.add(<HTMLElement>node.parentNode);
    });
